...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type=='rnn':
          ...
          grads['b_vocab']=np.sum(dx,axis=(0,1))
          dx_reshape=dx.reshape(-1,dx.shape[-1])
          h_reshape=h.reshape(-1,h.shape[-1])
          grads['W_vocab']=h_reshape.T@dx_reshape
          h_grad=dx@W_vocab.T
          dx,dh0,grads['Wx'],grads['Wh'],grads['b']=rnn_backward(h_grad,cache_rnn)
        ...
        if self.cell_type=='lstm':
          ...
          grads['W_vocab']=h_reshape.T@dx_reshape
          h_grad=dx@W_vocab.T
          dx,dh0,grads['Wx'],grads['Wh'],grads['b']=lstm_backward(h_grad,cache_rnn)
          grads['W_embed']=word_embedding_backward(dx,cache_embedding)
          grads['b_proj']=np.sum(dh0,axis=0)
          grads['W_proj']=features.T@dh0
