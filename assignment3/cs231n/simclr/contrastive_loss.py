...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs = torch.sum(out_left * out_right, 1, keepdim=True) / (torch.linalg.norm(out_left, dim=1, keepdim=True) * torch.linalg.norm(out_right, dim=1, keepdim=True))
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    numerator = exponential[torch.arange(N), torch.arange(N) + N]
    numerator = torch.cat((numerator, numerator)).reshape(-1, 1)
