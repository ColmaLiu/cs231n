...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        if self.normalization=='batchnorm':
            ...
            for i in range(1,self.num_layers):
                if i==1:
                    ...
                    out,cache=batchnorm_forward(out,self.params['gamma1'],self.params['beta1'],self.bn_params[0])
        ...
        if self.normalization=='layernorm':
            ...
            for i in range(1,self.num_layers):
                if i==1:
                    out,cache=affine_forward(X,self.params['W1'],self.params['b1'])
                    affine_cache.append(cache)
                    out,cache=layernorm_forward(out,self.params['gamma1'],self.params['beta1'],self.bn_params[0])
                    ...
                    out,cache=affine_forward(out,self.params['W'+str(i)],self.params['b'+str(i)])
                    affine_cache.append(cache)
                    out,cache=layernorm_forward(out,self.params['gamma'+str(i)],self.params['beta'+str(i)],self.bn_params[i-1])
                    layernorm_cache.append(cache)
                    out,cache=relu_forward(out)
                    relu_cache.append(cache)
                    if self.use_dropout:
        ...
        if self.normalization==None:
            dx,grads['W'+str(self.num_layers)],grads['b'+str(self.num_layers)]=affine_backward(dx,affine_relu_cache[-1])
            grads['W'+str(self.num_layers)]+=self.reg*self.params['W'+str(self.num_layers)]
            for i in range(self.num_layers-1,0,-1):
                if self.use_dropout:
                    dx=dropout_backward(dx,dropout_cache[i-1])
                dx,grads['W'+str(i)],grads['b'+str(i)]=affine_relu_backward(dx,affine_relu_cache[i-1])
                grads['W'+str(i)]+=self.reg*self.params['W'+str(i)]

        if self.normalization=='batchnorm':
            dx,grads['W'+str(self.num_layers)],grads['b'+str(self.num_layers)]=affine_backward(dx,affine_cache[-1])
            grads['W'+str(self.num_layers)]+=self.reg*self.params['W'+str(self.num_layers)]
            for i in range(self.num_layers-1,0,-1):
                if self.use_dropout:
                    dx=dropout_backward(dx.dropout_cache[i-1])
                dx=relu_backward(dx,relu_cache[i-1])
                dx,grads['gamma'+str(i)],grads['beta'+str(i)]=batchnorm_backward_alt(dx,batchnorm_cache[i-1])
                dx,grads['W'+str(i)],grads['b'+str(i)]=affine_backward(dx,affine_cache[i-1])
                grads['W'+str(i)]+=self.reg*self.params['W'+str(i)]
        ...
        if self.normalization=='layernorm':
            ...
            for i in range(self.num_layers-1,0,-1):
                if self.use_dropout:
                    dx=dropout_backward(dx,dropout_cache[i-1])
                dx=relu_backward(dx,relu_cache[i-1])
                dx,grads['gamma'+str(i)],grads['beta'+str(i)]=layernorm_backward(dx,layernorm_cache[i-1])
                dx,grads['W'+str(i)],grads['b'+str(i)]=affine_backward(dx,affine_cache[i-1])
                grads['W'+str(i)]+=self.reg*self.params['W'+str(i)]
